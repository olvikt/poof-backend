...
class CourierAvailableOfferResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        $windowFromAt = $this->asDateTime($order?->window_from_at);
        $windowToAt = $this->asDateTime($order?->window_to_at);
        $dispatchAvailableAt = $this->asDateTime($order?->dispatch_available_at);
        $finalMatchingStartsAt = $windowFromAt?->subMinutes((int) config('courier_runtime.scheduled_matching.lead_minutes', 30));

        $reservationStage = $this->resolveReservationStage($interest, $isAssigned, $now, $finalMatchingStartsAt);
        ...
        return [
            ...
            'offer_expires_at' => $this->asDateTime($this->expires_at)?->toISOString(),
            'seconds_remaining' => $secondsRemaining,
            'countdown_active' => $countdownActive,
            'countdown_started_at' => $countdownActive ? $this->asDateTime($this->created_at)?->toISOString() : null,
            'countdown_expires_at' => $countdownActive ? $this->asDateTime($this->expires_at)?->toISOString() : null,
            'service' => [
            ...
            'scheduled_window_label' => $this->windowLabel($windowFromAt, $windowToAt),
    ...
    private function resolveReservationStage(?CourierOrderInterest $interest, bool $isAssigned, DateTimeInterface $now, ?CarbonImmutable $finalMatchingStartsAt): string {
        ...
        if ($this->status === OrderOffer::STATUS_EXPIRED || ($this->expires_at && $this->asDateTime($this->expires_at)?->lessThanOrEqualTo($now))) return 'expired';
        ...
        if ($finalMatchingStartsAt && $finalMatchingStartsAt->lessThanOrEqualTo($now)) return 'final_matching';
    ...
    private function windowLabel(?CarbonImmutable $from, ?CarbonImmutable $to): ?string { if (! $from || ! $to) return null; return $from->format('H:i').'–'.$to->format('H:i'); }
}
