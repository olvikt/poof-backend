<div class="w-full px-4 py-4 text-white" style="padding-bottom: calc(var(--courier-nav-h) + env(safe-area-inset-bottom) + 7.5rem);" wire:poll.5s>
    ...
    @else
        ...
        <div class="mb-4 overflow-hidden rounded-3xl border border-white/8 bg-[#0d141e] shadow-[0_16px_36px_rgba(0,0,0,0.35)]">
            <div class="relative h-[50dvh] min-h-[360px] max-h-[560px] w-full overflow-hidden bg-[#0b131d]" data-map-bootstrap='@json($mapBootstrap ?? null)'>
                ...
                @unless($hasMapPreviewData)
                    <div class="absolute inset-0 flex items-center justify-center px-4">
                        <div class="rounded-2xl border border-white/12 bg-[#101722]/95 px-3 py-2 text-center text-xs text-slate-200">
        ...
        <div class="space-y-3">
            @foreach($orders as $order)
                ...
                <div wire:key="my-order-{{ $order->id }}" class="courier-surface border border-white/8 p-4" x-data="{ contactOpen: false, clientPhone: @js($clientPhone) }">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="mt-1 flex items-start justify-between gap-2">
                                ...
                                <div class="flex shrink-0 flex-wrap justify-end gap-1.5 text-[11px]">
                                    @if($distanceKm !== null)
                                        <span class="rounded-full border border-white/12 bg-white/[0.08] px-2 py-0.5 {{ $distanceKm <= 1 ? 'text-emerald-300' : ($distanceKm <= 3 ? 'text-amber-300' : 'text-orange-300') }}">{{ number_format($distanceKm, 1) }} км</span>
                                    @endif

                                    @if($etaMin !== null)
                                        <span class="rounded-full border border-white/12 bg-white/[0.08] px-2 py-0.5 text-slate-200">~{{ $etaMin }} хв</span>
                                    @endif

                                    @if($elapsedLabel)
                                        <span class="rounded-full border border-white/12 bg-white/[0.08] px-2 py-0.5 text-slate-200">{{ $elapsedLabel }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mt-3 rounded-2xl border border-white/6 bg-[#0d1522] p-3">
