    ...
    DEPLOY_STATE_PAYLOAD="$(cat <<STATE
{
  ...
  "rollback_source_release_ref": $(release_state_json_string_or_null "$PREVIOUS_RELEASE_REF" "$PHP_BIN"),
  "rollback_source_commit": $(release_state_json_string_or_null "$PREVIOUS_COMMIT" "$PHP_BIN"),
  "rollback_target_release_ref": "$RESOLVED_REF",
